...
def load_thresholds(path: Path | str = None) -> Dict[str, Any]:
    if path is None:
        # Default to resonance_thresholds.yaml in the same directory as this module
        path = Path(__file__).parent / "resonance_thresholds.yaml"
    else:
        path = Path(path)
